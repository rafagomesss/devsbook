...
if (document.querySelector('.fic-item-field')) {
    document.querySelectorAll('.fic-item-field').forEach(item => {
        item.addEventListener('keyup', async (e) => {
            if (e.keyCode == 13) { // quando for [Enter]
                let id = item.closest('.feed-item').getAttribute('data-id');
                let txt = item.value;
                item.value = '';
                ...
                if (json.error.length) {
                    let content = `<div class="flash">
                        <span style="font-family: Helvetica; font-weight: 500;">${json.error}</span>
                    </div>`;
                    item.closest('.feed-item')
                        .querySelector('.feed-item-comments-area')
                        .innerHTML += content;
                    let count = document.querySelectorAll('.flash').length;
                    let flash = document.getElementsByClassName('flash').item(count - 1);
                    if (flash) {
                        fade(flash);
                    }
                }
